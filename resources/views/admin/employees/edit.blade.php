...
@section('title',trans('employees.edit_employee'))
@section('content_header')
<div class="row mb-2">
    <div class="col-sm-6">
        <h1>
            <i class="fas fa-building" style="margin:10px 10px 10px 10px;"></i>@lang('employees.edit_employee')
        </h1>
    </div>
    <div class="col-sm-6">
                ...
                <div class="form-group col-xs-12 col-sm-12 col-md-4 col-lg-4">
                    ...
                    <select id="company_id" name="company_id" class="form-control select2 select2-hidden-accessible" data-placeholder="@lang('employees.company')" style="width:100%;">
                        ...
                        @foreach($companies as $company)
                            <option @if($employee->company_id == $company->id) selected="selected" @endif value="{{ $company->id }}">{{ $company->name }}</option>
                        @endforeach
                    </select>
                </div>                
            </div>
